...
# --------------------------------------------------------------
df = pd.read_pickle(
    r"C:\Users\Bilal's Desktop\Desktop\Machine learning Excersise\data-science-template\data\interim\01_data_processed.pkl"
)
...
df[outlier_columns[3:6] + ["label"]].plot.hist(
    by="label", figsize=(20, 20), layout=(3, 3)
)
...
for col in outlier_columns:
    for label in df["label"].unique():
        dataset = mark_outliers_chauvenet(
            outliers_removed_df[outliers_removed_df["label"] == label], col=col
        )
        # replace values marked as outliers with NaN in the original dataframe
        ...
        print(
            f"Number of outliers removed for column {col} and label {label}: {n_outliers}"
        )
